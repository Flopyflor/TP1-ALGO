...
\[w_n = w_0 * \prod\limits_{i=1}^{n}{t_i} + \sum\limits_{i=1}^{n-1}{(t_n * f_i * \prod\limits_{j=i+1}^{n-1}{t_j})} + f_n\]
\[w_n = w_0 * \prod\limits_{i=1}^{n}{t_i} + \sum\limits_{i=1}^{n-1}{(t_n * f_i * \prod\limits_{j=i+1}^{n-1}{t_j})} + f_n * \sum\limits_{i=1}^{0}{(t_n * f_i * \prod\limits_{j=i+1}^{n-1}{t_j})}\]
\[w_n = w_0 * \prod\limits_{i=1}^{n}{t_i} + \sum\limits_{i=1}^{n}{(f_i * \prod\limits_{j=i+1}^{n}{t_j})}\]
$Q.E.D$
...
\[f_0 = r_c *t_0\] 
caso 1:\\
\[f_1 = r_c *\prod\limits_{k=0}^{1}{t_k}\]
\[f_1 = r_c *\prod\limits_{k=0}^{1}{t_k}\] %TODO hace falta aclarar que se cumple la recursividad en un paso extra?
